...
class ChrdSupabaseUser extends BaseAuthUser {
  ChrdSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> chrdSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = ChrdSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
